                    ...
                    if let Some(prob) = prob {
                        ...
                        if relative_ne!(confidence / prob.abs(), 1.0, epsilon = 0.1) {
                            ...
                            if p > confidence {
                                ...
                                if rules.len() >= 2 {
                                    // Ignore the strongest rule only if it actively leads to bad decisions
                                    if let Some((rule, _)) = rules.into_iter().filter(|(_, p)| p.abs() > 0.5).max_by(|(_, a), (_, b)| match a.abs().partial_cmp(&b.abs()) {
